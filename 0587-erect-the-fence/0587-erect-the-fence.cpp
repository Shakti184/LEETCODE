class Solution {
public:
    int getRotAng(vector<int>A,vector<int>B,vector<int>C){
    ...
    vector<vector<int>> outerTrees(vector<vector<int>>& trees) {
        ...
        for(int i=2;i<trees.size();i++){
            int ls=lTrees.size();
            while(lTrees.size()>=2&&getRotAng(lTrees[ls-2],lTrees[ls-1],trees[i])>0){
                lTrees.pop_back();
                ls--;
            }
        ...
        for(int i=trees.size()-3;i>=0;i--){
            int rs=rTrees.size();
            while(rTrees.size()>=2&&getRotAng(rTrees[rs-2],rTrees[rs-1],trees[i])>0){
                rTrees.pop_back();
                rs--;
            }
